/**
 * Made using some code from the Raspberry Pi Pico examples
...
static void substep_set_calibration_data(substep_state_t *state, int step0, int step1, int step2)
{
    ...
    printf("calibration data set to: %d, %d, %d\n", step0, step1, step2);
}



// Constructor
ENC::ENC(uint8_t pin_a)
{
    PIO pio = pio0;
    const uint sm = 0;

    pio_add_program(pio, &quadrature_encoder_substep_program);
    substep_init_state(pio, sm, pin_a, &_state);


    pio_add_program(pio, &quadrature_encoder_substep_program);
    substep_init_state(pio, sm, pin_a, &_state);

    // example calibration code from example; DOES NOT WORK HERE:
...
//    set_pwm(0);
    ...
    substep_set_calibration_data(&_state, 85, 128, 215);
...
void ENC::update()
{
    ...
    substep_update(&_state);

    // Uncomment to calibrate, while running the motor at 50% PWM
    //substep_calibrate_phases(_state.pio, _state.sm);

    if (_last_position != _state.position || _last_speed != _state.speed || _last_raw_step != _state.raw_step) {
        // print out the result
        _last_position = _state.position;
        _last_speed = _state.speed;
        _last_raw_step = _state.raw_step;
    }
}